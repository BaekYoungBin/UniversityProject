...
<%@ page import="java.io.*"%>
	...
	%><form action="Agency_Reservation_Delete.jsp" method="POST" >
	...
	try{
		...
		ResultSet rs = stmt.executeQuery("select A_ID, Plane_type, Origin, Destination, F_time, Lead_time from Agency_Reservation where A_id = '"+ID+"'");
		int i = 0;
		while (rs.next()){		
			...
			%>	
			<tr><td><%=A_ID %></td><td><%=Plane_Type%></td><td><%=Origin%></td><td><%=Destination%></td>
